...
switch(end($uri)){
    ...
    if(empty($bodyAdatok["felhasznalonev"]) || empty($bodyAdatok["jelszo"]) || empty($bodyAdatok["email"]) ||empty($bodyAdatok["szamlazasicim"]) ||empty($bodyAdatok["rangId"]))
    {
        echo json_encode(["valasz"=>"Hianyzo adat!"],JSON_UNESCAPED_UNICODE);
        return http_response_code(400);
    }
    $ujFelhasznaloSQL="INSERT INTO felhasznalo VALUES (?,?,?,?,?)";
    $ujFelhasznalo=adatokValtoztatasa($ujFelhasznaloSQL,"ssssi",[$bodyAdatok["felhasznalonev"],$bodyAdatok["jelszo"],$bodyAdatok["email"],$bodyAdatok["szamlazasicim"],$bodyAdatok["rangId"]]);
    if($ujFelhasznalo){
    ...
    $modositFelhasznaloSQL="UPDATE felhasznalo SET jelszo=?, email=?, szamlazasicim=?,rangId=? WHERE felhasznalonev=?";
    $modositFelhasznalo=adatokValtoztatasa($modositFelhasznaloSQL,"sssis",[$bodyAdatok["jelszo"],$bodyAdatok["email"],$bodyAdatok["szamlazasicim"],$bodyAdatok["rangId"],$bodyAdatok["felhasznalonev"]]);
